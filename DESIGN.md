...
- **The metadata editor** — `flyleaf-core`, and from slice 5 of its extraction `flyleaf`, from `excelano/flyleaf`. Added 2026-09-07. The editor `§4` and `§5` describe was written here and is being moved out so that Tommy Flyleaf, a standalone TOML editor, and this application share one editor rather than two that drift; that repository's `PROMPT.md` holds the plan and `tests/golden/` here is what each step of it is measured against. A git dependency pinned by revision while the extraction ran, and from crates.io at 0.1 since the same day; the rule above that the library is never a path dependency holds for this one too. It depends on the same `toml_edit` major `slpc` re-exports and must, since a `DocumentMut` from one is handed to the other. Measured on adding it: one crate new in the tree, `flyleaf-core` itself, and `toml_edit` resolved once.
- **The desktop's light and dark setting, on Linux only** — `zbus`, pure Rust and no build script. Added 2026-08-28; `§3` says why it is needed and this entry says what it cost, which is nothing: `zbus` is already compiled on this target under `accesskit_unix`, which arrives through `egui-winit`. Measured by taking the crate list before and after — 275 crates both times, no name added. `blocking-api` is a feature of that same crate and generates code rather than pulling one, and `default-features` is off so `async-io` is named rather than taken, which is the feature `accesskit` already turns on.
...
**Nothing compiles C.** Across the fleet this is a preference rather than a rule since 2026-09-04, and `~/notes/pure_rust_preference.md` holds the stance and what taking C costs; this application keeps it as a rule for itself, because a build that needs a Rust toolchain and nothing else is what the rest of this section measures. `cc`, `cmake`, `pkg-config`, and `bindgen` are all absent from the build-dependency tree. `wayland-sys` and `linux-raw-sys` are declaration crates that resolve their symbols at run time. A build needs a Rust toolchain and nothing else.
...
**Amended 2026-08-28: the window follows the desktop's light and dark setting, and on one platform it has to ask for itself.** Nothing here had ever chosen a theme, on the reasoning that the toolkit reports what the desktop wants and egui follows it. That is true on two platforms of three. `winit` answers `system_theme()` from `should_use_dark_mode()` on Windows and from `NSApplication`'s `effectiveAppearance` on macOS, and returns `None` on Linux unconditionally — a body that is the word and nothing else — so egui fell back to `Theme::Dark` and every Linux user saw the dark window whatever their desktop was set to.

Measured before it was believed, because the comfortable explanations had to go first. With GNOME set to Light the card drew dark; with `color-scheme` forced to `prefer-light`, so the portal answered `2` rather than `0`, the card drew dark again **and the window's own titlebar turned light in the same screenshot**. One window, two halves, disagreeing about the theme — which rules out the desktop having failed to say what it wanted. Two builds a fortnight apart behaved identically, so it was never a regression.

`src/system_theme.rs` reads the XDG desktop portal's `org.freedesktop.appearance color-scheme` and then follows it, and is empty on the two platforms where the toolkit answers — pinning a preference there would replace a live answer with a stale one. It follows rather than merely reading once, because `ThemeChanged` gives the other two platforms that for free and Linux would otherwise be the one place where the answer goes stale the moment a person changes their mind.

**The judgement in it is what *no preference* means, and it is the whole of why this works on GNOME.** The specification defines 0 as no preference, 1 as prefer dark, 2 as prefer light. GNOME's Settings offers Light and Dark and spells them `default` and `prefer-dark`, so a person choosing Light produces 0 and never 2 — `prefer-light` has to be set by hand through `gsettings` and no part of the interface will do it. Reading 0 as *leave it dark* would therefore give every GNOME user who chose Light the dark card, which is the defect wearing a different hat. It is read as light, which is also what GTK does with `ADW_COLOR_SCHEME_DEFAULT`, so this application looks like its neighbours on that desktop rather than like the exception. The cost is a desktop that is genuinely dark while declaring no preference, where this now draws light — and where its GTK neighbours are drawing light too. Where the portal cannot be reached at all, nothing is chosen and egui's own fallback stands, because that is not a third answer.
...
**Amended: a string the tree will not let anybody edit is shown escaped.** SPEC §3 requires a member name be shown with the characters that reorder text escaped, and `slpc::display_name` does that for the card. The tree rendered every string straight into a `TextEdit`, so `payload.file` — which `is_protected` disables, making it a display rather than a field — read `reportfdp.exe` for a payload called `report<U+202E>fdp.exe`. egui gives a bidirectional formatting character zero advance width, so the tree was drawing the spoof the card two rows above it had escaped, in the one field nobody is allowed to change. Found by hand on Windows on 2026-08-29 while running the card's item 3, which asks about the card; macOS and Linux had both ticked it correctly without looking down. Fixed in `displayed`. **An editable string is deliberately left alone**, which is why the rule is about protected keys and not about names: escaping a value somebody can type into writes the escape back into their document the moment the field is touched, and §4's first sentence means the tree has no schema with which to tell a name from any other string. `CHECKLIST.md` holds the run and the two tests, each broken deliberately.
...
**Amended 2026-09-07: which keys are protected is the application's answer, not the tree's.** The tree carried the two required keys itself, and that was the one thing in it that was about Slipcase rather than about TOML. It now takes a policy, `RequiredKeys` in `src/lib.rs`, that answers whether a path is protected and how a protected string reads; the tree knows no key names. Done in place, before the tree moved out to `excelano/flyleaf`, so that the move was a move. Measured: the goldens in `tests/golden/` hold byte for byte.

**Amended 2026-09-07, evening: flyleaf 0.2 brings the general editor's behaviour here, on purpose.** Every row and section carries a small button reading its kind, the four datetime shapes told apart, whose menu offers the kinds the value can become and a comment to add; arrays are sections of editable elements rather than a leaf showing their text; every comment is a field, edited on blur and removed when emptied, above its key rather than joined into a label after the value; sections start closed above 200 entries, which no metadata document reaches. The two required keys stay protected as before, and their comments are editable, since protection is about a key's name and value. The goldens in `tests/golden/` were regenerated for all of it, by decision, in the commit that took 0.2.

**Amended the same day: the tree is `flyleaf::render` now.** `src/tree.rs` left this repository with its tests and became the widget in `excelano/flyleaf`; this section still describes what it draws, and the goldens still hold it to that. What this repository keeps is `RequiredKeys`, the one call, and the record of why every decision above was taken.
...
**Amended: the tree renders every entry, and that is why this application sets its own limit.** A `ScrollArea` that draws all of its contents is right for a metadata document — the format defines two keys and SPEC §2.2's example is four lines — and wrong for a document written to be expensive. Measured 2026-08-27 against the densest conformant metadata anybody can write, shortest legal keys and shortest legal values, with the window up: 135 MB resident for a container with no metadata to speak of, 347 MB for 256 KiB of it, 891 MB for a megabyte. About 8.7 KB for every key, of which roughly 1.3 KB is the parsed document and the rest is what `egui` retains for a row it has been shown. Every one of those containers is conformant.

`slpc` 0.3.7 bounds the metadata member at 1 MiB by default, chosen against what parsing costs. This application bounds it at 256 KiB, chosen against what rendering costs, which is the larger number and which the library has no way to know: `LIMITS` in `src/lib.rs` carries the measurement. A container over it is reported undetermined, which is SPEC §6's answer and is not a verdict against the file.

Switching to a clipped `ScrollArea` was the alternative and is not obviously wrong, only harder: `show_rows` wants a uniform row height, and this tree has sections, comment lines above a key, and rows that grow with what is typed into them. A bound is one constant and a measurement; the clipped tree is a rewrite of §4 to buy a limit that would still have to exist, because the parse is 1.3 KB of the 8.7 whatever draws it.
...
**Amended 2026-09-07: the metadata has undo and redo.** Found by hand the day flyleaf 0.2 arrived: a comment edited in the tree could not be taken back, and Ctrl+Z reached only egui's own undo of a focused field's text. The document is `flyleaf_core::Document` now, which holds the edited baseline `as_parsed` used to and a history beside it, a copy of the rendered text per step with edits to one row coalesced. The window records after every frame, takes Ctrl+Z and Ctrl+Shift+Z before anything draws so a field cannot answer them, and tells the widget to forget what is half typed first, for the reason its own record gives. Undo and Redo sit beside Save. The baseline is built from the tree's text rather than the container's bytes, for the reason the field it replaces recorded: two conformant containers do not re-serialize to their bytes.
...
**Amended: saving was laundering too, and every amendment above was about the other door.** Everything from here down reasons about extraction — a payload leaving a container, and what the copy carries. Saving an edit is the same question in the other direction and nobody asked it. `Destination::in_place` replaces a container by renaming a fresh file over it, and a fresh file carries no mark, so editing one key in a container that had been downloaded and pressing Save stripped whatever the platform had recorded about where it came from. Measured 2026-08-27 on Linux, where the mark is a note: `arrived_from_elsewhere` went from true to false across a save. On Windows it is Mark of the Web and on macOS the attribute this document already records as the difference between an unsigned application from the internet running and being stopped, and the loss then propagates — every payload extracted afterwards is unmarked too, because carrying copies from the container. True from the day `in_place` existed.

The fix is in the library, where `CLAUDE.md` requires it and where every caller who replaces a file wants it: `slpc` 0.3.7's `commit` carries the mark onto the replacement before the rename, and `fs` now implies `provenance` so that asking for the ability to replace a container cannot quietly mean asking for the bug. `src/staging.rs`'s macOS arm does not use `in_place` and so does not get the fix; it carries the mark itself, onto the staged file before `replaceItemAtURL:`, rather than trusting that call to preserve an attribute nobody here has measured it preserving.
...
**Amended: decided, and the card says it.** The line is *The payload is an executable file; the extracted copy will not be executable.* It appears when the payload member's stored mode carries an execute bit and not otherwise, which means it is silent for a container written by a tool that records no mode at all. Silent where the container does not say, which is the shape `§3` already requires of the platform's silence. Gated to Unix, because a mode bit is not what makes a file executable on Windows and a sentence about one would be false there.

Three alternatives were rejected and one of them is the tempting one. **Preserving the execute bit on extraction is not the fix it looks like.** Under the sandbox the extracted copy is quarantined whatever its mode, so Gatekeeper refuses it as *damaged* either way and the bit changes nothing about the message a person sees; what it would change is that opening a container could produce an executable file, which is the wrong direction for an application whose provenance rule exists to stop a payload arriving as something this machine made. It trades a confusing sentence for a hazard. **Disabling the Open button for such a payload** is the move `§5` already declined for provenance, on the grounds that substituting this application's judgement about danger for the platform's is what `§3` refuses to do about type. **And doing nothing** leaves a person advised to bin a file that is fine, when the container held the fact that explains it.

What the line deliberately does not do is predict a platform's message. It says what is true of the file. `§8`'s amendment records what it costs to write a claim about a return value as though it were a claim about the screen, and that mistake is not worth making twice in the same document.

It needed the library first. `slpc` read `unix_mode()` and kept only the file-type bits as `EntryKind`; the permission bits were discarded and there was no accessor, so the fact this line rests on was unreachable. That was a library addition rather than something to work around here, which `CLAUDE.md` requires and which this repository has now done five times. `Container::payload_mode` landed in `slpc` 0.3.6 on 2026-08-27.

**Amended: the silence is not `unix_mode()`'s doing, and this paragraph credited it to the wrong thing.** It argued that a container recording no mode is silent because `unix_mode()` returns an `Option` and a ZIP from a non-Unix writer has none. The `Option` is real and the inference was wrong: measured in `zip` 8.6, an archive made on DOS gets a mode invented for it — `S_IFREG | 0o664`, or `0o444` where the read-only bit is set — and only an archive whose external attributes are entirely zero comes back `None`. Had the accessor been built on `unix_mode()`, every container written by a Windows tool would have been told its payload is not executable, which is a confident answer to a question the container never answered and is the one thing this line must not do. `payload_mode` reads the external attributes off the central directory instead and answers only where the high sixteen bits carry something. The line is silent for the reason claimed; the mechanism is the library's and had to be built rather than found.

**Amended: the policy moved into the library, and the amendments above stay as the record of how it was reached.** Everything from here to the top of this run of amendments was worked out in this repository, and none of it is wrong. What changed is where it lives. Reviewing `slipcase unpack` from this side turned up that the command-line tool over the same library had been laundering provenance since its 0.1.0 — the identical defect, in the sibling that does the identical operation, because the code was here and nothing called it from there. `slpc` 0.3.5 carries `provenance::carry`, `provenance::arrived_from_elsewhere` and `Mark`, and `src/provenance.rs` is gone from this repository. The library's own DESIGN §4.7 supplies the test for whether it belonged there — *a defect held once is fixed where a defect copied is rediscovered* — and the proof of copying was already on the record: the reasoning error about payload names had by then been written independently in both repositories in nearly the same words, and in `SECURITY.md` a third time.

Three things this cost or bought that a reader should not have to reconstruct. `xattr` is a dev-dependency here now rather than a dependency, since only a test reaches for it. The eighteen tests that lived beside the module went with it, which was not automatic — deleting the module dropped this suite from 74 to 67 and the library had three tests where this repository had eighteen, so the zone-gating boundary and the sandbox-mark predicate would have been left uncovered in both places at once. And what stays here is the *use*: `copy_out` still decides that a payload whose provenance could not be carried is not left on disk, and the card still decides to report rather than gate. The library holds the mechanism and the rule about what an error means; what this application does about it is still §5's to say.
...
**Amended: the package carries no maintainer scripts.** `desktop-file-utils` and `hicolor-icon-theme` own dpkg triggers on the two directories the package writes into, so the caches are rebuilt without a `postinst` asking. They are dependencies for that as much as for anything they provide at run time. `shared-mime-info` was a third until the media type moved to `slipcase-common`, which pulls it in.
