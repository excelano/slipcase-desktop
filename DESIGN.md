...
**Implements:** Slipcase specification 1.1, through the `slpc` library rather than directly.
...
A desktop application that opens a `.slpc` file, shows its flyleaf, hands the content file to whatever application the operating system has registered for it, and makes a container out of a file a person chooses. It is presented to a person as **Slipcase**; the crate and the binary are `slipcase-desktop`, so nothing on `PATH` collides with the command-line tool.
...
- **The flyleaf editor** — `flyleaf` and `flyleaf-core`, from `excelano/flyleaf`, from crates.io. One editor across the Excelano applications rather than two that drift. It depends on the same `toml_edit` major that `slpc` re-exports and must, since a `DocumentMut` from one is handed to the other.
...
- **Launching the content file** — `opener`, which hands a path to the platform's own mechanism.
...
**The flyleaf is the window.** It gets the space rather than a panel down one side.

**The content file is a card**: its filename, its size, what the operating system says would open it, and the three things that can be done with it. **Open** extracts it and launches whatever the platform registered. **Extract** puts it where the user says and launches nothing. **Replace** puts another file in its place. It is not previewed.

**Showing a container asks for focus on Open**, so a double-click followed by Enter is the whole of the interaction. The focus is asked for once and only where the button is enabled: every frame would pin it there and leave the tree and the Save unreachable from the keyboard, and a focus ring on a disabled button says press me about a button that cannot be pressed. Opening the content file automatically was rejected — it would make a container an autorun archive, which is the thing store review exists to catch.
...
**The card asks before it offers.** Where this build cannot decode the content file, it says so in the card and does not offer the two actions that would have to. Replace stays, because writing over a member does not read it.
...
**An integer is not right-aligned.** Every value starts in the same column. Right-aligning a number puts it against the window's edge, far from its own key and from every value above and below it, and knowing which key a number belongs to is worth more in a flyleaf document than arithmetic legibility.
...
**New container… asks which file goes in and where the container goes, and nothing else.** It packs through `slpc::pack_reader` and shows the result. The flyleaf it writes is empty, and the two keys SPEC §2.2 requires are the library's to fill in. What a person wants to say about the content file they say in the tree afterwards: §4's first sentence is that there is no schema here, so a form asking for anything else would be this application inventing a vocabulary the specification does not define.

Neither question may be guessed. Which file goes in is the whole of what is being asked for, and where the container lands is a location — `slipcase pack` defaults to writing beside the content file, which is a convention a command line can afford and a button cannot. The second dialog is a save dialog, so the platform asks before overwriting and the answer to that question is the only permission this needs. The naming convention is offered rather than imposed: the dialog is prefilled with the content file's name and `.slpc` after it. Nothing here reads a container's name to find out what is inside it.

**The content file's mark comes with it, and without that, packing is a way to launder a download.** A container this process writes carries no mark of its own, so packing a gated file, opening the container — which the card would then call local — and pressing Open would hand the operating system an unmarked copy of a file it had gated, every step being somebody's ordinary use of the window. `create` carries the content file's mark onto the container, and `tests/handover.rs` walks the whole trip rather than the container alone, because what matters is the file the platform is handed at the end of it. A carry that fails is said and not fatal, which is this section's decision for `staging.rs` too: what opens a container is this application, which reports provenance rather than acting on it.
...
**The content file: extract and replace, as explicit actions.** No temporary-file watching and no save interception. The user says when they are done and the application does not guess.

**A replacement waits for a Save.** Choosing the file is not writing it. It waits beside the flyleaf edits so that one press writes one container. Writing on the press would rewrite the same archive twice with a window between the two where a failure leaves half of what was asked for, and it would put an arbitrarily large write inside a file dialog.
...
| No flyleaf member, or more than one | non-conformant | the verdict |
| Flyleaf present, not UTF-8 or not TOML | non-conformant | the verdict |
| Flyleaf parses, a required key is absent | non-conformant | the verdict, and the tree |
| `content.file` names no member, or more than one | non-conformant | the verdict, and the tree |
| `content.file` is not a name a content file may have | non-conformant | the verdict, and the tree |
| The flyleaf member cannot be read at all | undetermined | the verdict, and nothing further |
| A `slipcase_version` this build does not implement | out of scope | the verdict, and the tree |
| Conformant, content file of zero length | conformant | everything, size stated plainly |
| Conformant, content file very large | conformant | everything, extraction that can be waited on |
| Conformant, content file encrypted or compressed by a method this build lacks | conformant | everything, and a refusal where the content file would have been |

**Every tree above comes from `slpc::flyleaf_of`**, which parses the flyleaf member alone; `Container::read` fails the content check before it yields a document. The verdict comes from `slpc::validate`.

**The library reaches every one of these and the application renders it.** A container whose flyleaf cannot be read may not be reported conformant or non-conformant, and one declaring a version this build does not implement may not be reported conformant to a version it does.

**A content file of zero length is conformant** under SPEC §2.3 and the card says nothing about it beyond its size. **A very large content file** makes extraction work with a duration, so the Open button reports progress and can be cancelled.

**A conformant container's content file may still be out of reach.** SPEC §2.5 puts encryption and compression method outside conformance, so a container carrying an encrypted content file is sound and its bytes cannot be had. The card describes it in full, because the name and the size come from the central directory and need no decoder, and it states the refusal before anything is pressed: `Container::check_content_readable` answers from that same entry, so asking costs nothing. Open and Extract are not offered; Replace is. The corpus holds the two answers against each other — every content file the card refuses must fail to extract in the same words, and every content file it offers must extract.
...
**Extraction addresses a path the platform will accept.** A content file name can be legal, safe to join, and still not a file: Win32 resolves `CON`, `COM1`, `AUX`, `LPT1`, `PRN` and `NUL` to devices wherever they appear, so `File::create` returns `Ok`, `write_all` returns `Ok`, and no file exists afterwards, while `std::fs::read` never returns at all — it opens the console for reading and waits for input a windowed application will never supply. A path in the `\\?\` verbatim form is not parsed that way, so `slpc::content_path` builds one from a canonicalized directory and the name stops being a device without anybody deciding it is a bad name. Nothing here holds a list of reserved names: which names are devices is Windows's to know.

It does not touch `extract_at`, because that path is one a person typed and §5 says the two halves of extraction differ in whose name it is. And it does not make the content file openable: `opener::open` on a device-named file fails with *the specified device name is invalid*, which is an error the application has a sentence for.
...
An application bundle with `CFBundleDocumentTypes` and an exported type declaration conforming to `public.zip-archive`, which a container is. The role is `Editor` rather than `Viewer`, because §5 writes edited flyleaf back, and it is bounded, since the only type claimed is the one the bundle exports.
