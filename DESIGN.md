...
It needs the library first. `slpc` reads `unix_mode()` and keeps only the file-type bits as `EntryKind`; the permission bits are discarded and there is no accessor, so the fact this line rests on is not currently reachable. That is a library addition rather than something to work around here, which `CLAUDE.md` requires and which this repository has now done four times.
